...
func main() {
	username := flag.String("username", "", "director username")
	password := flag.String("password", "", "director password (minimum 8 characters)")
	...
	if *username == "" || *password == "" || *employeeCode == "" || *prefix == "" || *firstName == "" || *lastName == "" {
		flag.Usage()
		log.Fatal("all seed-director flags are required")
	}
	...
	fmt.Printf("director created: %s (%s)\n", user.Username, user.UID)
}
