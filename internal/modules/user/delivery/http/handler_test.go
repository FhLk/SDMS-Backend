...
type userUsecaseStub struct {
	...
	resetPasswordFn func(context.Context, uuid.UUID, usecase.ResetPasswordInput) error
...
func (s *userUsecaseStub) UpdateStatus(ctx context.Context, id uuid.UUID, input usecase.UpdateUserStatusInput) (*domain.User, error) {
	if s.updateStatusFn != nil {
		return s.updateStatusFn(ctx, id, input)
	}
	return nil, errors.New("unexpected UpdateStatus call")
}

func (s *userUsecaseStub) ResetPassword(ctx context.Context, id uuid.UUID, input usecase.ResetPasswordInput) error {
	if s.resetPasswordFn != nil {
		return s.resetPasswordFn(ctx, id, input)
	}
...
func TestUserHandlerCreate(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		service := &userUsecaseStub{createFn: func(_ context.Context, input usecase.CreateUserInput) (*domain.User, error) {
			if input.Username != "somchai" || input.EmployeeCode != "EMP-001" || input.Prefix != "นาย" ||
				input.FirstName != "สมชาย" || input.LastName != "ใจดี" || input.Role != domain.RoleTeacher || input.Password != "password123" {
				t.Errorf("input = %+v", input)
			}
			return sampleUser(id), nil
		}}
		body := []byte(`{"username":"somchai","employee_code":"EMP-001","prefix":"นาย","first_name":"สมชาย","last_name":"ใจดี","role":"TEACHER","password":"password123"}`)
	...
	t.Run("invalid JSON", func(t *testing.T) {
		...
		if got := decodeUserBody[ErrorResponse](t, resp); got.Message != "invalid request body" {
			t.Errorf("response = %+v", got)
		}
	})
	...
	t.Run("usecase error is mapped", func(t *testing.T) {
		...
		if got := decodeUserBody[ErrorResponse](t, resp); got.Message != "username already exists" {
			t.Errorf("response = %+v", got)
		}
	})
}
...
func TestUserHandlerList(t *testing.T) {
	...
	got := decodeUserBody[[]UserResponse](t, resp)
	if len(got) != 1 || got[0].UID != id || got[0].EmployeeCode != "EMP-001" {
		t.Errorf("response = %+v", got)
	}
...
func TestUserHandlerGetByID(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		if got := decodeUserBody[UserResponse](t, resp); got.UID != id {
			t.Errorf("response = %+v", got)
		}
	})
	...
	t.Run("invalid ID", func(t *testing.T) {
		...
		if got := decodeUserBody[ErrorResponse](t, resp); got.Message != "invalid user id" {
			t.Errorf("response = %+v", got)
		}
	})
	...
	t.Run("not found", func(t *testing.T) {
		...
		if got := decodeUserBody[ErrorResponse](t, resp); got.Message != "user not found" {
			t.Errorf("response = %+v", got)
		}
	})
}
...
func TestUserHandlerGetByUsernameRoutePrecedesIDRoute(t *testing.T) {
	...
	if got := decodeUserBody[UserResponse](t, resp); got.Username != "somchai" {
		t.Errorf("response = %+v", got)
	}
...
func TestUserHandlerUpdate(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		service := &userUsecaseStub{updateFn: func(_ context.Context, gotID uuid.UUID, input usecase.UpdateUserInput) (*domain.User, error) {
			...
			user := sampleUser(id)
			user.Prefix, user.Role = input.Prefix, input.Role
			return user, nil
		}}
		...
		if got := decodeUserBody[UserResponse](t, resp); got.Prefix != "ดร." || got.Role != "DIRECTOR" {
			t.Errorf("response = %+v", got)
		}
	})
	...
	t.Run("usecase error", func(t *testing.T) {
		service := &userUsecaseStub{updateFn: func(context.Context, uuid.UUID, usecase.UpdateUserInput) (*domain.User, error) {
			return nil, domain.ErrEmployeeCodeAlreadyExists
		}}
		resp := userRequest(t, newUserTestApp(service), nethttp.MethodPut, "/api/v1/users/"+id.String(), body)
		defer resp.Body.Close()
		if resp.StatusCode != fiber.StatusConflict {
...
func TestUserHandlerUpdateStatus(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		service := &userUsecaseStub{updateStatusFn: func(_ context.Context, gotID uuid.UUID, input usecase.UpdateUserStatusInput) (*domain.User, error) {
			if gotID != id || input.Status != domain.StatusInactive {
				t.Errorf("id/input = %s, %+v", gotID, input)
			}
			user := sampleUser(id)
		...
		if got := decodeUserBody[UserResponse](t, resp); got.Status != "INACTIVE" {
			t.Errorf("response = %+v", got)
		}
	})
	...
	t.Run("usecase error", func(t *testing.T) {
		service := &userUsecaseStub{updateStatusFn: func(context.Context, uuid.UUID, usecase.UpdateUserStatusInput) (*domain.User, error) {
...
func TestUserHandleErrorMappings(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := decodeUserBody[ErrorResponse](t, resp); got.Message != tt.wantBody {
				t.Errorf("response = %+v", got)
			}
		})
	}
...
func TestNewUserListResponseReturnsEmptyArrayForEmptyInput(t *testing.T) {
	...
	if got == nil || len(got) != 0 {
		t.Fatalf("newUserListResponse(nil) = %#v", got)
	}
}
